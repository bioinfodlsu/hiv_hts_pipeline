...
ruleorder: last_score_training > last_score_training_SE
ruleorder: align_last > align_last_SE
ruleorder: align_last_trained > align_last_trained_SE
...
rule last_frag_stat_est: #Rule for estimating length distribution of paired-end reads
    input:
        ...
        reads1 = "{0}".format(config['out_dir'])+"/filtered_reads/{sample_id}/{sample_id}_1_subsampled.fq",
        reads2 = "{0}".format(config['out_dir'])+"/filtered_reads/{sample_id}/{sample_id}_2_subsampled.fq"
    params:
...
rule last_score_training: #Rule for training score parameters
    input:
        reference_flag = "{0}".format(config["out_dir"])+"/last_index/{sample_id}_index.done",
        query = "{0}".format(config['out_dir'])+"/filtered_reads/{sample_id}/{sample_id}_1_subsampled.fq",
    params:
        last_index_basename="{0}".format(config["out_dir"])+"/last_index/{sample_id}_index",
...
rule align_last: #Rule for aligning paired-end reads to a reference genome, with score training
    ...
    params:
        ...
        mean = getMean,
        std = getSTD,
        match = lambda wildcards: config["aligner_params_dict"][wildcards.param_group]["match"],
        mismatch = lambda wildcards: config["aligner_params_dict"][wildcards.param_group]["mismatch"],
        gapOpen = lambda wildcards: config["aligner_params_dict"][wildcards.param_group]["gapOpen"],
        gapExtend = lambda wildcards: config["aligner_params_dict"][wildcards.param_group]["gapExtend"],
    threads:
        workflow.cores/len(config["reads"])
    output:
    ...
    shell:
        """
        fastq-interleave {input.reads1} {input.reads2} |
        lastal -Q1 -i1 -r {params.match} -q {params.mismatch} -a {params.gapOpen} -b {params.gapExtend} {params.last_index_basename} |
        last-pair-probs -f {params.mean} -s {params.std} -m 0.01 -d 0.1 |
        maf-convert sam -f {input.dict} > {output}
        """

rule last_score_training_SE: #Rule for training score parameters
    input:
        reference_flag = "{0}".format(config["out_dir"])+"/last_index/{sample_id}_index.done",
    ...
    params:
        ...
        # file_type=file_type
    output:
        "{0}".format(config["out_dir"])+"/last_score_sample/{sample_id}_scoring_scheme_single"
    conda:
       "../envs/last.yaml"
    shell:
        """
...
rule align_last_trained_SE:
    input:
        reference_flag = "{0}".format(config["out_dir"])+"/last_index/{sample_id}_index.done",
        scoring =  "{0}".format(config["out_dir"])+"/last_score_sample/{sample_id}_scoring_scheme_single",
        query = "{0}".format(config['out_dir'])+"/filtered_reads/{sample_id}/{sample_id}_subsampled.fq",
        dict = "{0}".format(config["out_dir"])+"/filtered_reads/{sample_id}/{sample_id}.dict",
...
rule align_last_SE: #Rule for aligning paired-end reads to a reference genome, with score training
    ...
    shell:
        ...
        # """
        """
        lastal -Q1 -i1 -r {params.match} -q {params.mismatch} -a {params.gapOpen} -b {params.gapExtend} {params.last_index_basename} {input.query} |
        last-map-probs -m 0.95 |
        maf-convert sam -f {input.dict} > {output}
        """
