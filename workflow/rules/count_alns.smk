...
rule count_last_alns:
    ...
    output:
        temp("{0}".format(config['out_dir'])+"/last_alignments/to_{reference_name}/last_counts_alns.tsv")
    conda:
        "../envs/count.yaml"
    shell:
...
rule count_last_trained_alns:
    ...
    output:
        temp("{0}".format(config['out_dir'])+"/last_trained_alignments/to_{reference_name}/last_trained_counts_alns.tsv")
    conda:
        "../envs/count.yaml"
    shell:
        "python workflow/scripts/count_alns.py --inFile {input} --outFile {output}"
...
rule count_bowtie2_alns:
    ...
    output:
        temp("{0}".format(config['out_dir'])+"/bowtie2_alignments/to_{reference_name}/bowtie2_counts_alns.tsv")
    conda:
        "../envs/count.yaml"
    shell:
        "python workflow/scripts/count_alns.py --inFile {input} --outFile {output}"
...
rule compare_alns:
    ...
    output:
        "{out_dir}/alignment_analysis/to_{reference_name}/plot_alns_{country}.tsv"
    threads:
...
rule plot_alns:
    input:
        "{out_dir}/alignment_analysis/to_{reference_name}/plot_alns.tsv"
    output:
        "{out_dir}/alignment_analysis/to_{reference_name}/alignments_comparison.png"
    threads:
        workflow.cores/len(config["reads"])
    conda:
        "../envs/count.yaml"
