...
rule count_last_trained_alns:
    input:
        aln_files = expand("{out_dir}/last_trained_alignments/{sample_id}.sam",
...
rule count_last_alns:
    input:
        aln_files=expand("{out_dir}/last_alignments/{sample_id}/{params}/alns.sam",
                         out_dir = config["out_dir"],
                         sample_id=config["reads"].keys(),
                         params=last_paramspace.instance_patterns)
    output:
        "{0}".format(config['out_dir'])+"/last_alignments/last_counts_alns"
    run:
        out = open(output[0],"w")
