rule convertSAM:
    ...
    shell:
        """
        samtools view -S -bh {input.sam} > {output}
        """
...
rule sortBAM:
    input:
        "{0}".format(config['out_dir'])+"/{aligner}_alignments/{country}/{sample_id}_to_{reference_name}/paramgroup_{param_group}/alns.bam"
    output:
...
rule indexBAM:
    input:
        "{0}".format(config['out_dir'])+"/{aligner}_alignments/{country}/{sample_id}_to_{reference_name}/paramgroup_{param_group}/alns.sorted.bam"
    output:
        "{0}".format(config['out_dir'])+"/{aligner}_alignments/{country}/{sample_id}_to_{reference_name}/paramgroup_{param_group}/alns.sorted.bai"
    conda:
        "../envs/last.yaml"
    shell:
